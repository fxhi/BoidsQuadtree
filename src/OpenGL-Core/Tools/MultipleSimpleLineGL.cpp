#include "MultipleSimpleLineGL.hpp"
...
MultipleSimpleLineGL::MultipleSimpleLineGL(std::vector<float> linesPoints, float width)
    : m_linesPoints(linesPoints), m_width(width) {
    m_linesPoints = linesPoints;
    createVAO();
}

MultipleSimpleLineGL::~MultipleSimpleLineGL() {
    glDeleteVertexArrays(1, &VAO);
    glDeleteBuffers(1, &VBO);
    glDeleteBuffers(1, &EBO);
}

void MultipleSimpleLineGL::draw() {
...
std::vector<float> MultipleSimpleLineGL::getVertices(float x1, float y1, float x2, float y2) {
    glm::vec2 p1(x1,y1);
    glm::vec2 p2(x2,y2);
...
std::vector<unsigned int> MultipleSimpleLineGL::getIndices(unsigned int pos) {
...
void MultipleSimpleLineGL::createVAO() {
