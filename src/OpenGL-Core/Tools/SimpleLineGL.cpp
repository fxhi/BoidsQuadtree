...
SimpleLineGL::SimpleLineGL(float x1, float y1, float x2, float y2, float width) : m_p1(x1, y1), m_p2(x2, y2), m_width(width) {
    createVAO();
}

SimpleLineGL::SimpleLineGL() {
    createVAO();
}
...
std::vector<float> SimpleLineGL::getVertices() {
    glm::vec2 p1p2;
    if (m_p1.y <= m_p2.y) {
        std::swap(m_p1, m_p1);
    }
...
std::vector<unsigned int> SimpleLineGL::getIndices() {
